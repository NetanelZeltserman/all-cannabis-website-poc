...
import getRelatedProducts from '../api/product/related-products';
import { NavigationButtons } from '../components/navigation/navigation-buttons';
import { Product } from '../components/product/product';
import ProductCard from '../components/product/product-card';
import Spinner from '../components/spinner';

export default function RelatedProducts({ productSlug }: { productSlug: string }) {
  ...
  useEffect(() => {
    const fetchProducts = async () => {
      setIsLoading(true);
      try {
        const fetchedProducts = await getRelatedProducts(productSlug);
  ...
  return (
    <div dir="ltr" className="w-full px-8 py-16 bg-gray-100">
      <h2 className="mb-8 text-3xl font-bold text-center">מוצרים קשורים</h2>
      ...
      {!isLoading && products.length > (isMobile ? 1 : 3) && (
